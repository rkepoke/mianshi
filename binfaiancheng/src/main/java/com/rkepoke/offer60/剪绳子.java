...
public class 剪绳子 {
    public int cutRope(int target) {
        ...
        if(target==2){
            return 1;//因为一定要切，最少一刀，所以值为1*1
        }
        if(target==3){
            return 2;//1*2
        }
        int [] result=new int[target+1];//定义一个数组用来存储不同长度绳子切后乘积的最大值，这么定义的初衷，方便利用数组下标
        // 最重要的一个理念就是不断划分为小问题，找到最合适的解
        result[1]=1;//这是用来存储长度的，1号位存储长度为1
        result[2]=2;//                    2               2
        result[3]=3;
        for(int i=4;i<=target;i++){//能到这一步，target的长度最起码为4
            int max=0;
            for(int j=1;j<=i/2;j++){//一次探索在每个长度下，那种乘积是最大值  比如：4，要尝试1*3；2*2；结束，去最大值，其他同理
